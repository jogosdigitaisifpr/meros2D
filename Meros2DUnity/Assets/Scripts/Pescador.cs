...
public class Pescador : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "Player" && !hasTriggered )
        {
            ...
            if (SceneManager.GetActiveScene().name == "fase1_2" && collision.gameObject.GetComponent<PlayerCollision>().contaCutPescMal == 0)
            {
                collision.gameObject.GetComponent<PlayerCollision>().contaCutPescMal++;
                hasTriggered = true;
